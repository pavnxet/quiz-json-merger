...
function mergeJsonFiles(fileContents) {
  const allQuestions = [];
  const stats = [];
  for (const { name, content } of fileContents) {
    try {
      ...
      let count = 0;
      if (Array.isArray(parsed)) { count = parsed.length; allQuestions.push(...parsed); }
      else if (typeof parsed === 'object' && parsed !== null) { count = 1; allQuestions.push(parsed); }
      stats.push({ file: name || 'telegram_upload', count });
    } catch { throw new Error(`Invalid JSON in: ${name || 'unknown'}`); }
  }
  if (allQuestions.length === 0) throw new Error('No valid questions found');
  return { merged: JSON.stringify(allQuestions, null, 2), total: allQuestions.length, stats };
}
...
async function handleWebUpload(request) {
  try {
    ...
    const { merged, total, stats } = mergeJsonFiles(fileContents);
    ...
    return new Response(blob, {
      headers: {
        ...
        'X-Merge-Stats': JSON.stringify({ files: stats, total })
      }
    });
...
async function processTelegramMessage(msg, env) {
  ...
  try {
    ...
    if (msg.document) {
      ...
      return tgSend(api('sendMessage'), { chat_id: chatId, text: "⚠️ Please send only `.json` files." });
    }
  } catch (err) {
...
async function processMerge(chatId, session, env, api) {
  ...
  const { merged, total, stats } = mergeJsonFiles(fileContents);
  const statsText = stats.map(s => `📄 ${s.file}: ${s.count} Qs`).join('\n');
  const caption = `✅ Merged successfully!\n📊 ${statsText}\n💡 Total: ${total} questions`;
  ...
  <div class="container">
    ...
    <form id="mergeForm">
      ...
      <div id="drop-zone" class="drop-zone">
        ...
        <input type="file" id="files" name="files" multiple accept=".json,application/json" hidden>
      </div>
